...
# 内核构建 (独立发布 @sole03/rule-engine-core)
npm run build:core
```
...
governflow/
├── packages/
│   ├── core/                    # @sole03/rule-engine-core — 协议无关内核
│   │   └── src/
...
│   │       ├── cli/             # CLI 入口
│   │       └── di/              # 依赖注入容器
│   └── dashboard/               # @sole03/governflow-dashboard — 可视化仪表盘
...
`@sole03/rule-engine-core` 零 MCP 依赖，可独立发布为 npm 包：

```bash
npm install @sole03/rule-engine-core
```

```typescript
import { MerlionBridge, RegoCompiler, CanaryController } from "@sole03/rule-engine-core";
```
